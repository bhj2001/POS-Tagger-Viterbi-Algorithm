...
def assign_POS_tags(test_words, dict2_tag_follow_tag, dict2_word_tag):
	...
	for sentence in test_words:
		...
		for tag in list_of_tags:
			try:
				dp[0][tag]=dict2_word_tag[sentence[0]][tag]
			except KeyError:
				if tag=='N':
					dp[0][tag]=1/list_of_tags['N']
				else:
					dp[0][tag]=0
		...
		for i in range(1,len(sentence)):
			for cur_tag in list_of_tags:
				...
				for prev_tag in list_of_tags:
					tmp=dp[i-1][prev_tag]*dict2_tag_follow_tag[prev_tag][cur_tag]
					if tmp>maxi:
						maxi=tmp
						max_ans=prev_tag
				try:
					dp[i][cur_tag]=maxi*dict2_word_tag[sentence[i]][cur_tag]
				except KeyError:
					if cur_tag=='N':
						dp[i][cur_tag]=maxi/list_of_tags['N']
					else :
						dp[i][cur_tag]=0
